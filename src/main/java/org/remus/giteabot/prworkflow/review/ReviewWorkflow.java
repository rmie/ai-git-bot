...
public class ReviewWorkflow implements PrWorkflow {
    ...
    public WorkflowResult run(PrWorkflowContext context) {
        ...
        return switch (action) {
            case ACTION_REVIEW -> doReview(context);
            case ACTION_BOT_COMMAND -> doBotCommand(context);
            case ACTION_INLINE_COMMENT -> doInlineComment(context);
            case ACTION_REVIEW_SUBMITTED -> doReviewSubmitted(context);
            case ACTION_PR_CLOSED -> doPrClosed(context);
